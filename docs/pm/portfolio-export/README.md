...
- [x] Screenshots captured via Playwright (`e2e/portfolio-screenshots.spec.ts`, 2026-07-28): login, today, today-mobile, progress, jobs, review, roadmap, settings. **PM** (`gyam-pm.png`) added to the spec 2026-08-17 — recapture locally after LAN has `/pm`; do not commit live PNGs.  
- [x] Recapture against live LAN 2026-08-17 (local files only; **not** in git). User **Sean**. Jobs are real. Login PIN field empty.  
- [ ] Recapture **PM** (`gyam-pm.png`) after `/pm` is on LAN — local only; funnel is counts, still do not commit if other tiles show private notes  
...
- [ ] Explicit “what I did as PM” bullet list (supervised agent team, scope control, RAID, `/pm` teaching dashboard)
...
4. Approach — hybrid Agile + SoT + agent RACI + in-app `/pm` (honest charts, no fake EVM)  
5. Outcomes — V1 shipped; Playwright; living docs; S10 PM dashboard + How this works  
